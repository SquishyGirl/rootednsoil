( function( api ) {
	...
	api.controlConstructor['parvati-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.parvati-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the parvati_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'parvati_get_all_google_fonts_ajax',
								parvati_customize_nonce: parvati_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.parvati-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.parvati-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.parvati-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.parvati-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.parvati-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.parvati-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.parvati-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.parvati-font-variant' );
			...
			control.container.on( 'change', '.parvati-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.parvati-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.parvati-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.parvati-font-family select' ).selectWoo();
	jQuery( '.parvati-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".parvati-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.parvati-font-variant' ).hide();
		}
	});
