	...
	function parvati_enqueue_google_fonts() {
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				$variants = parvati_get_google_font_variants( $parvati_settings[$key], $key );
		...
		$default_fonts = PARVATI_DEFAULT_FONTS;
...
if ( ! function_exists( 'parvati_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'parvati_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function parvati_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'parvati_fonts_customize_register' ) ) {
			// Bail if Parvati Premium is active
			return;
		}
		...
		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Parvati_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'parvati_sanitize_variants'
			)
		);
		...
		$wp_customize->add_control(
			new Parvati_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'parvati' ),
					'settings' => array(
		...
		$wp_customize->add_control(
			new Parvati_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'parvati' ),
					'section' => 'font_section',
					'settings' => array(
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'type' => 'option',
				'sanitize_callback' => 'parvati_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Parvati_Range_Slider_Control(
				...
				array(
					'type' => 'parvati-range-slider',
					'description' => __( 'Line height', 'parvati' ),
					'section' => 'font_section',
					'settings' => array(
		...
		$wp_customize->add_control(
			new Parvati_Range_Slider_Control(
				...
				array(
					'type' => 'parvati-range-slider',
					'description' => __( 'Paragraph margin', 'parvati' ),
					'section' => 'font_section',
					'settings' => array(
...
if ( ! function_exists( 'parvati_get_all_google_fonts_ajax' ) ) {
	...
	function parvati_get_all_google_fonts_ajax() {
		...
		if ( ! isset( $_POST[ 'parvati_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'parvati_customize_nonce' ] ), 'parvati_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'parvati_customize_nonce', 'parvati_customize_nonce' );
...
if ( ! function_exists( 'parvati_get_google_font_variants' ) ) {
	 ...
	 *
	 */
	function parvati_get_google_font_variants( $font, $key = '' ) {
...
if ( ! function_exists( 'parvati_get_google_font_category' ) ) {
	 ...
	 */
	function parvati_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'parvati_get_font_family_css' ) ) {
	...
	function parvati_get_font_family_css( $font, $settings, $default ) {
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . parvati_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'parvati_add_to_font_customizer_list' ) ) {
	add_filter( 'parvati_typography_customize_list', 'parvati_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function parvati_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = parvati_get_all_google_fonts();
		$select_fonts = parvati_get_all_google_fonts( apply_filters( 'parvati_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $parvati_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $parvati_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $parvati_settings[ $setting ], parvati_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $parvati_settings[ $setting ] ) ) ] = array(
				'name' => $parvati_settings[ $setting ],
		...
		if ( function_exists( 'parvati_secondary_nav_get_defaults' ) ) {
			...
			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], parvati_typography_default_fonts() ) ) {
