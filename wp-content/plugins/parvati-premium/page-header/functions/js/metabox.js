jQuery(document).ready(function($) {
	$( '#parvati-tabs-container input[type="checkbox"]' ).lc_switch( '', '' );

	$( '.parvati-tabs-menu a' ).on( 'click', function( event ) {
		event.preventDefault();
		$( this ).parent().addClass( 'parvati-current' );
		$( this ).parent().siblings().removeClass( 'parvati-current' );
		var tab = $( this ).attr( 'href' );
		$( '.parvati-tab-content' ).not(tab).css( 'display', 'none' );
		$( tab ).fadeIn( 100 );

		if ( '#parvati-image-tab' == tab || '#parvati-content-tab' == tab ) {
	...
	$( '#_meta-parvati-page-header-content' ).bind( 'input change', function() {
...
jQuery(window).load(function($) {
	if ( jQuery( '#_meta-parvati-page-header-enable-image-crop' ).val() == 'enable' ) {
		jQuery( '#crop-enabled' ).show();
	}

	jQuery( '#_meta-parvati-page-header-enable-image-crop' ).change(function () {
	...
	if ( jQuery( '#_meta-parvati-page-header-image-background' ).is( ':checked' ) ) {
	...
	jQuery('body').delegate('.image-background', 'lcs-statuschange', function() {
		if (jQuery(this).is(":checked")) {
			...
			jQuery('.parallax').hide();
			jQuery('#_meta-parvati-page-header-image-background-fixed').lcs_off();
		}
	});

	if ( jQuery('#_meta-parvati-page-header-full-screen').is(':checked')) {
		jQuery('.vertical-center').show();
	} else {
		jQuery('.vertical-center').hide();
	}

	jQuery('body').delegate('#_meta-parvati-page-header-full-screen', 'lcs-statuschange', function() {
		if (jQuery(this).is(":checked")) {
			jQuery('.vertical-center').show();
		} else {
			jQuery('.vertical-center').hide();
			jQuery('#_meta-parvati-page-header-vertical-center').lcs_off();
		}
	});

	if ( jQuery('#_meta-parvati-page-header-transparent-navigation').is(':checked')) {
		jQuery('.navigation-colors').show();
	} else {
		jQuery('.navigation-colors').hide();
	}

	jQuery('body').delegate('#_meta-parvati-page-header-transparent-navigation', 'lcs-statuschange', function() {
		if (jQuery(this).is(":checked")) {
			jQuery('.navigation-colors').show();
		} else {
			jQuery('.navigation-colors').hide();
	...
	if ( jQuery('#_meta-parvati-page-header-combine').is(':checked')) {
		jQuery('.combination-options').show();
	} else {
		jQuery('.combination-options').hide();
	}

	jQuery('body').delegate('#_meta-parvati-page-header-combine', 'lcs-statuschange', function() {
		if (jQuery(this).is(":checked")) {
			jQuery('.combination-options').show();
		} else {
			jQuery('.combination-options').hide();
	...
	if ( jQuery('#_meta-parvati-page-header-image-background-type').val() == '' ) {
		jQuery('.vertical-center').hide();
		jQuery('.fullscreen').hide();
	}
	jQuery('#_meta-parvati-page-header-image-background-type').change(function () {
		if (jQuery(this).val() === '') {
			jQuery('.vertical-center').hide();
			jQuery('#_meta-parvati-page-header-vertical-center').lcs_off();
			jQuery('.fullscreen').hide();
			jQuery('#_meta-parvati-page-header-full-screen').lcs_off();
		} else {
			//jQuery('.vertical-center').show();
	...
	var $set_button = jQuery('.parvati-upload-file');
	/**
...
jQuery(document).ready(function($) {
	$('#parvati-tabs-container .color-picker').wpColorPicker();

	jQuery( '.parvati-page-header-remove-image' ).on( 'click', function( e ) {
		e.preventDefault();
		...
		if ( '-1' == jQuery( '#_thumbnail_id' ).attr( 'value' ) ) {
			...
			jQuery( '.parvati-page-header-set-featured-image' ).show();
	...
	$('#postimagediv').on( 'click', '#remove-post-thumbnail', function() {
		...
		if ( '' == jQuery( '#_meta-parvati-page-header-image-id' ).attr( 'value' ) ) {
			jQuery( '.page-header-image-settings' ).hide();
			jQuery( '.parvati-page-header-set-featured-image' ).show();
		}
	...
	$( '.parvati-featured-image, .parvati-page-header-set-featured-image a' ).on( 'click', function( event ) {
		event.preventDefault();
