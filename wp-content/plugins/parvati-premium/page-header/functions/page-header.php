...
if ( ! function_exists( 'parvati_get_blog_page_header' ) ) {
	/**
	 ...
	 */
	function parvati_get_blog_page_header() {
		$page_header = ( is_home() ) ? true : false;
		return apply_filters( 'parvati_get_blog_page_header', $page_header );
	}
}

if ( ! function_exists( 'parvati_pageheader_area_start_container' ) ) {
	/**
	 ...
	 */
	function parvati_pageheader_area_start_container( $image_class, $content_class ) {
		...
		$parallax_speed = apply_filters( 'parvati_pageheader_parallax_speed', 2 );
		...
		if ( $video_enabled && '' !== $options[ 'content' ] ) {
			...
			$video_options[ 'className' ] = 'className:parvati-page-header-video';

			if ( apply_filters( 'parvati_pageheader_video_loop', true ) ) {
				$video_options[ 'loop' ] = 'loop:true';
			} else {
				$video_options[ 'loop' ] = 'loop:false';
			}

			if ( apply_filters( 'parvati_pageheader_video_muted', true ) ) {
		...
		$combined_content = ( '' !== $options[ 'merge' ] ) ? ' parvati-combined-page-header' : '';
		...
		if ( '' !== $options[ 'content' ] && false !== $options[ 'content' ] ) {
			printf(
				...
				$content_class . $parallax . $full_screen . $vertical_center_container . $container_type . $combined_content . ' parvati-page-header parvati-content-header',
				( 'fluid' !== $options[ 'container_type' ] ) ? $video : null,
				$vertical_center,
				( '' !== $options[ 'merge' ] ) ? 'parvati-merged-header' : '',
				( ! empty( $parallax ) ) ? 'data-parallax-speed="' . esc_attr( $parallax_speed ) . '"' : '',
				$options[ 'page_header_id' ]
			);
		}

		do_action( 'parvati_inside_merged_page_header' );
	}
}

if ( ! function_exists( 'parvati_pageheader_area' ) ) {
	/**
	 ...
	 */
	function parvati_pageheader_area( $image_class, $content_class ) {
		// Get our options
		$options = parvati_pageheader_get_options();
		...
		do_action( 'parvati_before_page_header' );
		...
		if ( '' == $options[ 'content' ] && parvati_pageheader_get_image( 'ALL' ) ) {
			...
			printf(
				...
				esc_attr( $image_class ) . $inner_container . ' parvati-page-header',
		...
		if ( '' !== $options[ 'content' ] && false !== $options[ 'content' ] ) {
			...
			// If we were merging, the container would be added in the parvati_before_header hook
			if ( '' == $options[ 'merge' ] ) {
				parvati_pageheader_area_start_container( 'page-header-image', 'page-header-content' );
			}

			// Replace any found template tags
			$options[ 'content' ] = parvati_pageheader_template_tags( $options[ 'content' ] );

			// Print the rest of our page header HTML
			// The starting elements are inside parvati_pageheader_area_start_container()
			printf ( '<div class="page-header-content-wrapper %1$s %2$s">
				...
				( '' !== $options[ 'merge' ] ) ? 'parvati-combined-content' : '',
				$inner_container,
				( '' !== $options[ 'merge' ] ) ? 'parvati-inside-combined-content' : 'parvati-inside-page-header-content',
		...
		do_action( 'parvati_after_page_header' );
	}
}
