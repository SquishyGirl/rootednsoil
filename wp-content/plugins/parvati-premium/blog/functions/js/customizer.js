...
( function( $ ) {
	...
	$( 'body' ).on( 'parvati_spacing_updated', function() {
		if ( $( '.masonry-container' )[0] ) {
			var $initiate = jQuery('.masonry-container').imagesLoaded( function() {
				$container = jQuery('.masonry-container');
				if (jQuery($container).length) {
	...
	$container.on( 'load.infiniteScroll', function( event, response, path ) {
		...
		if ( wp.customize.value('parvati_blog_settings[column_layout]')() ) {
			$posts.addClass( 'parvati-columns' );
			$posts.addClass( 'grid-parent' );
			$posts.addClass( 'grid-' + wp.customize.value('parvati_blog_settings[columns]')() );
			$posts.addClass( 'tablet-grid-50' );
			$posts.addClass( 'mobile-grid-100' );
		} else {
			$posts.removeClass( 'parvati-columns' );
			$posts.removeClass( 'grid-parent' );
			$posts.removeClass( 'grid-' + wp.customize.value('parvati_blog_settings[columns]')() );
			$posts.removeClass( 'tablet-grid-50' );
			$posts.removeClass( 'mobile-grid-100' );
		}

		if ( wp.customize.value('parvati_blog_settings[masonry]')() ) {
			$posts.addClass( 'masonry-post' );
		} else {
			$posts.removeClass( 'masonry-post' );
		}

		if ( ! wp.customize.value('parvati_blog_settings[post_image_padding]')() ) {
