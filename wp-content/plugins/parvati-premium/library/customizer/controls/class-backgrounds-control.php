...
if ( class_exists( 'WP_Customize_Control' ) ) {
	class Parvati_Background_Images_Customize_Control extends WP_Customize_Control {
		public $type = 'gp-background-images';

		public function enqueue() {
			wp_enqueue_script( 'gp-backgrounds-customizer', trailingslashit( plugin_dir_url( __FILE__ ) )  . 'js/backgrounds-customizer.js', array( 'customize-controls' ), PARVATI_PREMIUM_VERSION, true );
		}

		public function to_json() {
			parent::to_json();

			$this->json[ 'position_title' ] = esc_html__( 'left top, x% y%, xpos ypos (px)', 'parvati-premium' );
			$this->json[ 'position_placeholder' ] = esc_html__( 'Position', 'parvati-premium' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.repeat ) ) { #>
				<div class="parvati-backgrounds-repeat">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.size ) ) { #>
				<div class="parvati-backgrounds-size">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.attachment ) ) { #>
				<div class="parvati-backgrounds-attachment">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.position ) ) { #>
				<div class="parvati-backgrounds-position">
					<label>
		...
		public function get_repeat_choices() {
			return array(
				'' => esc_html__( 'Repeat', 'parvati-premium' ),
				'repeat-x' => esc_html__( 'Repeat x', 'parvati-premium' ),
				'repeat-y' => esc_html__( 'Repeat y', 'parvati-premium' ),
				'no-repeat' => esc_html__( 'No Repeat', 'parvati-premium' )
			);
		}

		public function get_size_choices() {
			return array(
				'' => esc_html__( 'Size (Auto)', 'parvati-premium' ),
				'100' => esc_html__( '100% Width', 'parvati-premium' ),
				'cover' => esc_html__( 'Cover', 'parvati-premium' ),
				'contain' => esc_html__( 'Contain', 'parvati-premium' )
			);
		}

		public function get_attachment_choices() {
			return array(
				'' => esc_html__( 'Attachment', 'parvati-premium' ),
				'fixed' => esc_html__( 'Fixed', 'parvati-premium' ),
				'local' => esc_html__( 'Local', 'parvati-premium' ),
				'inherit' => esc_html__( 'Inherit', 'parvati-premium' )
			);
		}
	}
}
