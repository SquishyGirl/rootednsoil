...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Pro_Typography_Customize_Control' ) ) :
class Parvati_Pro_Typography_Customize_Control extends WP_Customize_Control
{
    public $type = 'gp-pro-customizer-typography';

	public function enqueue() {
		wp_enqueue_script( 'parvati-pro-typography-selectWoo', trailingslashit( plugin_dir_url( __FILE__ ) )  . 'js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), PARVATI_PREMIUM_VERSION, true );
		wp_enqueue_style( 'parvati-pro-typography-selectWoo', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'css/selectWoo.min.css', array(), PARVATI_PREMIUM_VERSION );

		wp_enqueue_script( 'parvati-pro-typography-customizer', trailingslashit( plugin_dir_url( __FILE__ ) )  . 'js/typography-customizer.js', array( 'customize-controls', 'parvati-pro-typography-selectWoo' ), PARVATI_PREMIUM_VERSION, true );

		wp_enqueue_style( 'parvati-pro-typography-customizer', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'css/typography-customizer.css', array(), PARVATI_PREMIUM_VERSION );
	}

	public function to_json() {
		parent::to_json();

		$number_of_fonts = apply_filters( 'parvati_number_of_fonts', 200 );
		$this->json[ 'default_fonts_title'] = __( 'System Fonts', 'parvati-premium' );
		$this->json[ 'google_fonts_title'] = __( 'Google Fonts', 'parvati-premium' );
		$this->json[ 'google_fonts' ] = apply_filters( 'parvati_typography_customize_list', parvati_get_all_google_fonts( $number_of_fonts ) );
		$this->json[ 'default_fonts' ] = parvati_typography_default_fonts();
		$this->json[ 'family_title' ] = esc_html__( 'Font family', 'parvati-premium' );
		$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'parvati-premium' );
		$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'parvati-premium' );
		$this->json[ 'category_title' ] = '';
		$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'parvati-premium' );
	...
	public function content_template() {
		...
		<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
			...
			#>
			<div id={{{ data.variant.id }}}" class="parvati-font-variant" data-saved-value="{{ data.variant.value }}">
				<label>
		...
		<# if ( 'undefined' !== typeof ( data.category ) ) { #>
			<div class="parvati-font-category">
				<label>
		...
		<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
			<div class="parvati-font-transform">
				<label>
	...
	public function get_font_weight_choices() {
		return array(
			'normal' => esc_html__( 'normal', 'parvati-premium' ),
			'bold' => esc_html__( 'bold', 'parvati-premium' ),
			'100' => esc_html( '100' ),
			'200' => esc_html( '200' ),
			'300' => esc_html( '300' ),
			'400' => esc_html( '400' ),
	...
	public function get_font_transform_choices() {
		return array(
			...
			'capitalize' => esc_html__( 'capitalize', 'parvati-premium' ),
			'uppercase' => esc_html__( 'uppercase', 'parvati-premium' ),
			'lowercase' => esc_html__( 'lowercase', 'parvati-premium' ),
		);
	}
}
endif;
