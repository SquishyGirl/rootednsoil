...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Customize_Spacing_Slider_Control' ) ) :
/**
 * Create our container width slider control
 */
class Parvati_Customize_Spacing_Slider_Control extends WP_Customize_Control
{
	// Setup control type
	...
	public function to_json() {
		...
		$this->json[ 'reset_title' ] = esc_attr__( 'Reset','parvati-premium' );
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'gp-spacing-customizer', trailingslashit( plugin_dir_url( __FILE__ ) )  . 'js/spacing-customizer.js', array( 'customize-controls' ), PARVATI_SPACING_VERSION, true );
		wp_enqueue_style( 'gp-spacing-customizer-controls-css', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'css/customizer.css', array(), PARVATI_SPACING_VERSION );
		
		wp_enqueue_script( 'jquery-ui-core' );
		wp_enqueue_script( 'jquery-ui-slider' );
		
		wp_enqueue_script( 'parvati-spacing-slider-js', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'js/spacing-slider.js', array( 'jquery-ui-slider' ), PARVATI_SPACING_VERSION );
		
		wp_enqueue_style('parvati-ui-slider', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'css/jquery-ui.structure.css');
		wp_enqueue_style('parvati-flat-slider', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'css/range-slider.css');
		
	}
}
endif;

if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Spacing_Customize_Control' ) ) :
/* 
 * Add our control for our padding options 
 */ 
class Parvati_Spacing_Customize_Control extends WP_Customize_Control {
	public $type = 'spacing';
	public $description = '';
	
	public function enqueue() {
		wp_enqueue_script( 'gp-spacing-customizer', plugin_dir_url( __FILE__ )  . 'js/spacing-customizer.js', array( 'customize-controls' ), PARVATI_SPACING_VERSION, true );
	}
	...
	public function content_template() {
		...
		<label>
			...
			<input class="parvati-number-control" type="number" style="text-align: center;" {{{ data.link }}} value="{{{ data.value }}}" />
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Spacing_Customize_Misc_Control' ) ) :
/* 
 * Add a class to display headings 
 */ 
class Parvati_Spacing_Customize_Misc_Control extends WP_Customize_Control {
    public $settings = 'parvati_spacing_headings';
    public $description = '';
	public $areas = '';
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Backgrounds_Customize_Control' ) ) :

class Parvati_Backgrounds_Customize_Control extends WP_Customize_Control {
	public function render() {}
}
endif;

if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Backgrounds_Customize_Misc_Control' ) ) :
/*
 * No longer used
 * Kept for back compat purposes 
 */
class Parvati_Backgrounds_Customize_Misc_Control extends WP_Customize_Control {
	public function render() {}
}
endif;

if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Blog_Customize_Control' ) ) :
/**
 * Add our number input field for the featured image width 
 */
class Parvati_Blog_Customize_Control extends WP_Customize_Control {
	public $type = 'gp-post-image-size';
	public $placeholder = '';
	
	public function enqueue() {
		wp_enqueue_script( 'gp-blog-customizer', trailingslashit( plugin_dir_url( __FILE__ ) ) . 'js/blog-customizer.js', array( 'customize-controls' ), PARVATI_BLOG_VERSION, true );
	}
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Blog_Number_Customize_Control' ) ) :
/**
 * Add a regular number input control 
 */
class Parvati_Blog_Number_Customize_Control extends WP_Customize_Control {
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Post_Image_Save' ) ) :
/**
 * Add a button to initiate refresh when changing featured image sizes 
 */
class Parvati_Post_Image_Save extends WP_Customize_Control {
	public function render() {}
}
endif;

if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Blog_Text_Control' ) ) :
/**
 * Add a control to display simple text 
 */
class Parvati_Blog_Text_Control extends WP_Customize_Control {
	public function render() {}
}
endif;

if ( ! class_exists( 'Parvati_Customize_Alpha_Color_Control' ) ) :
/**
 */
class Parvati_Customize_Alpha_Color_Control extends WP_Customize_Control {
	public function render() {}
}
endif;

if ( ! class_exists( 'Parvati_Copyright_Textarea_Custom_Control' ) ) :
/**
 * Class to create a custom tags control 
 */
class Parvati_Copyright_Textarea_Custom_Control extends WP_Customize_Control {
	public function render() {}
}
endif;

if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Parvati_Blog_Page_Header_Image_Save' ) ) :
/**
 * Add a control without a button to refresh the frame
 * This kicks in our image dimension settings
 *
 */
class Parvati_Blog_Page_Header_Image_Save extends WP_Customize_Control {
	public $type = 'page_header_image_save';
	
	public function to_json() {
		parent::to_json();
		$this->json[ 'text' ] = __( 'Apply image sizes','parvati-premium' );
	}
...
if ( ! class_exists( 'Parvati_Hidden_Input_Control' ) ) :
/**
 * Create our hidden input control 
 */
class Parvati_Hidden_Input_Control extends WP_Customize_Control
{
	// Setup control type
...
if ( ! class_exists( 'Parvati_Text_Transform_Custom_Control' ) ) :
/**
 * A class to create a dropdown for text-transform 
 */
class Parvati_Text_Transform_Custom_Control extends WP_Customize_Control
{
...
if ( ! class_exists( 'Parvati_Font_Weight_Custom_Control' ) ) :
/**
 * A class to create a dropdown for font weight 
 */
class Parvati_Font_Weight_Custom_Control extends WP_Customize_Control
{
    public function __construct($manager, $id, $args = array(), $options = array())
    {
        parent::__construct( $manager, $id, $args );
