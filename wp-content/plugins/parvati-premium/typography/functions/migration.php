<?php
if ( ! function_exists( 'parvati_typography_convert_values' ) ) {
	add_action( 'admin_init', 'parvati_typography_convert_values' );
	/**
	 ...
	 */
	function parvati_typography_convert_values() {
		...
		if ( 'true' == get_option( 'parvati_update_premium_typography' ) ) {
			return;
		}
		...
		if ( ! empty( $new_settings ) ) {
			$parvati_new_typography_settings = wp_parse_args( $new_settings, $parvati_settings );
			update_option( 'parvati_settings', $parvati_new_typography_settings );
		}

		// All done, set an option so we don't have to do this again
		update_option( 'parvati_update_premium_typography', 'true' );
	}
}

if ( ! function_exists( 'parvati_typography_convert_secondary_nav_values' ) ) {
	add_action( 'admin_init', 'parvati_typography_convert_secondary_nav_values' );
	/**
	 * Take the old secondary navigation font value and strip it of variants
	 * This should only run once.
	 *
	 */
	function parvati_typography_convert_secondary_nav_values() {
		...
		if ( 'true' == get_option( 'parvati_update_secondary_typography' ) ) {
			return;
		}
		...
		$value = $parvati_secondary_nav_settings[ 'font_secondary_navigation' ];
		...
		if ( ! empty( $new_settings ) ) {
			$parvati_new_typography_settings = wp_parse_args( $new_settings, $parvati_secondary_nav_settings );
			update_option( 'parvati_secondary_nav_settings', $parvati_new_typography_settings );
		}

		// All done, set an option so we don't go through this again
		update_option( 'parvati_update_secondary_typography','true' );
	}
}
...
if ( ! function_exists( 'parvati_typography_set_font_data' ) ) {
	add_action( 'admin_init', 'parvati_typography_set_font_data' );
	/**
	 ...
	 *
	 */
	function parvati_typography_set_font_data() {
		// Bail if we don't have our defaults
		if ( ! function_exists( 'parvati_get_default_fonts' ) ) {
			return;
		}
		...
		foreach( $font_settings as $setting ) {
			...
			if ( function_exists( 'parvati_secondary_nav_get_defaults' ) && 'font_secondary_navigation' == $setting ) {
				$parvati_settings = wp_parse_args(
					get_option( 'parvati_secondary_nav_settings', array() ),
					parvati_secondary_nav_get_defaults()
				);
			...
			if ( $defaults[ $setting ] == $parvati_settings[ $setting ] ) {
				continue;
			}

			// Don't need to continue if we're using a system font or our default font
			if ( in_array( $parvati_settings[ $setting ], parvati_typography_default_fonts() ) || 'Open Sans' == $parvati_settings[ $setting ] ) {
				continue;
			}
